<!-- Section 7 -->

<section id="hp-section-7" class="hp-section-7">

  <h3 class="hp-section-7-heading"><?php the_field( 'hp_section_7_heading' ); ?></h3>

  <section class="hp-section-7-cards-wrapper">

    <section class="section-7-card">

      <div class="section-7-check-icon">
        <img src="https://sb-assessment.local/wp-content/uploads/2021/12/check.svg" alt="">
      </div>

     <div class="section-7-card-img">
       <img src="https://sb-assessment.local/wp-content/uploads/2021/12/medical.svg" alt="">
     </div>

     <p class="section-7-card-text">
      <?php the_field( 'hp_section_7_card_1_text' ); ?>
     </p>

    </section>

    <section class="section-7-card">

      <div class="section-7-check-icon">
        <img src="https://sb-assessment.local/wp-content/uploads/2021/12/check.svg" alt="">
      </div>

     <div class="section-7-card-img">
       <img src="https://sb-assessment.local/wp-content/uploads/2021/12/lock.svg" alt="">
     </div>

     <p class="section-7-card-text">
      <?php the_field( 'hp_section_7_card_2_text' ); ?>
     </p>

    </section>

  </section>

  <nav class="section-7-button-wrapper">

    <button type="button" name="button">
      <?php the_field( 'hp_section_7_button_text' ); ?>
    </button>

  </nav>

</section>
